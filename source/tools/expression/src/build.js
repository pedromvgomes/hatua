 ...
 * diffing instead, so CI verifies the committed artefacts are current and
 * passing, not merely one of the two.
 */
...
function generate(lang, target) {
  ...
  if (lang === 'go') {
    ...
    prependSuppressions(path.join(target.staging, 'parser.js'))
...
/**
 * Peggy's output carries helpers it does not always call, and string literals
 * dense enough to read as high-entropy secrets. This repo's own biome.json
 * already excludes the generated directory, but bulwark's TypeScript gate lints
 * with its own Biome config and has no per-project ignore surface — so the only
 * lever is a suppression inside the file, which means the generator has to put
 * it there. Written before biome formats the staging directory, so the drift
 * check sees the same bytes on every run.
 */
function prependSuppressions(file) {
  const suppressions = [
  ...
  fs.writeFileSync(file, `${suppressions}\n${fs.readFileSync(file, 'utf8')}`, 'utf8')
}

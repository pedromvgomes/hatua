...
export const step = z.strictObject({
   ...
   * Hatua treats most verbs as opaque, but interprets five structurally: `core.fork` creates branches, `core.for_each` nests and exposes `item`, `core.repeat` nests and carries an `until:` condition, `core.map` derives its outputs from its own `entries` field rather than from its manifest, and `core.set_var` writes one of the Board's `vars` and is typed by that var's declaration rather than by a manifest field.
   * The three nesting verbs drive reference scope and derived layout; `core.map` drives reference scope alone; `core.set_var` drives neither and is here because its `value` is a Slot no manifest can type.
  ...
  /**
   * Loop children, nested directly with no branch wrapper. On `core.for_each` and `core.repeat`.
   */
  get steps() {
